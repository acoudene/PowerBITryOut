...
public class PowerBICustomizedClient
{
  private readonly PbiEmbedService _pbiEmbedService;

  public PowerBICustomizedClient(PbiEmbedService pbiEmbedService)
  {
    _pbiEmbedService = pbiEmbedService ?? throw new ArgumentNullException(nameof(pbiEmbedService));
  }
  ...
  public async Task<string?> FindParameterValueAsync(string tenantId, string reportName, string parameterName, CancellationToken cancellationToken = default)
  {
    ...
    return await _pbiEmbedService.FindParameterValueInDatasetInGroupAsync(
      groupName,
      dataSetName,
      parameterName,
      cancellationToken);
